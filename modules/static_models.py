...
from torch_geometric.utils import from_scipy_sparse_matrix,dropout_adj
from torch_geometric.nn import NNConv,SAGEConv,GATv2Conv
...
class StaticGATV2(nn.Module):
    def __init__(self,edge_index_mp,in_channel,aggr,embedding_dict,args_config,data_config,device):
        super(StaticGATV2).__init__()
        # data 
        self.n_users = data_config['n_users']
        self.n_items = data_config['n_items']
        self.edge_index = edge_index_mp
        ...
        self.affinity_score = MergeLayer(self.emb_size, self.emb_size,self.emb_size,1)
    ...
    def init_layers(self):
        ...
        for k in range(self.context_hops):
            if k == self.context_hops-1:
                conv_layer.append(GATv2Conv(in_channels=self.emb_size,out_channels=self.emb_size,\
                    heads=1,dropout=0.6,concat=False).to(self.device))
            else:
                conv_layer.append(GATv2Conv(in_channels=self.emb_size,out_channels=self.emb_size,\
                    heads=1,dropout=0.6,concat=False).to(self.device))
    ...
    def gcn(self, edge_dropout=True, mess_dropout=True):
        ...
        if self.edge_dropout:
            edge_index,_ = dropout_adj(self.edge_index,p=self.edge_dropout_rate,training=self.training)
        else:
        ...
        for k in range(self.context_hops):
            ...
            ego_embeddings = self.conv_layer[k](ego_embeddings,edge_index.to(self.device))
            #
...
class GAT(nn.Module):
    def __init__(self, data_config, args_config, adj_mat,edge_index=None):
        ...
        if edge_index is  None:
            self.edge_index,_ = from_scipy_sparse_matrix(adj_mat) # for pyg layers
        else:
        ...
        *********************************************************
        ...
        """
        self.embedding_dict, self.conv_layer = self.init_weight()
    ...
    def init_weight(self):
        ...
        for k in range(self.context_hops):
            if k == self.context_hops-1:
                conv_layer.append(GATConv(in_channels=self.emb_size,out_channels=self.emb_size,\
                    heads=1,dropout=0.6,concat=False).to(self.device))
            else:
                conv_layer.append(GATConv(in_channels=self.emb_size,out_channels=self.emb_size,\
                    heads=1,dropout=0.6,concat=False).to(self.device))
            
        return embedding_dict, conv_layer
...
class GraphSAGE(nn.Module):
    ...
    def init_weight(self):
        ...
        self.embedding_dict = {
            'user_emb': initializer(torch.empty(self.n_users,
                                                 self.emb_size)).to(self.device),
            'item_emb': initializer(torch.empty(self.n_items,
                                                 self.emb_size).to(self.device))
        }
        ...
        for k in range(self.context_hops):
            ...
            self.conv_layer.append(SAGEConv(in_channels=self.emb_size,out_channels=self.emb_size,root_weight=False).to(self.device)) 
    ...
    def gcn(self, edge_dropout=True, mess_dropout=True):
        ...
        ego_embeddings = torch.cat([self.embedding_dict['user_emb'].clone(),
                                    self.embedding_dict['item_emb'].clone()], 0)
    ...
    def pooling(self, embeddings):
        ...
        if self.pool == 'mean':
            ...
            return embeddings[:,1,:] # 0 is before the first layer so no sense to take it
        else:  # final
            return embeddings[:, -1, :]
